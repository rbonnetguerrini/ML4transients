...
python /sps/lsst/users/rbonnetguerrini/ML4transients/scripts/convert_lc_for_snn.py "$INPUT_DIR" "$CSV_DIR"
if [ $? -ne 0 ]; then
...
mkdir -p "$INFER_OUT"
python /sps/lsst/users/rbonnetguerrini/ML4transients/scripts/inference_snn.py "$CSV_DIR" "$INFER_OUT"
if [ $? -ne 0 ]; then
...
python /sps/lsst/users/rbonnetguerrini/ML4transients/scripts/save_inference_to_h5.py "$INFER_OUT" "$INPUT_DIR"
