...
def create_batch_configs(base_config_path, batch_size=50):
    ...
    configs_dir = Path("configs/data_preparation/batches")
...
if __name__ == "__main__":
    base_config = sys.argv[1] if len(sys.argv) > 1 else "configs/data_preparation/configs_cutout.yaml"
    batch_size = int(sys.argv[2]) if len(sys.argv) > 2 else 50
    
    batch_configs, num_batches = create_batch_configs(base_config, batch_size)
    
    print(f"\nCreated {len(batch_configs)} batch config files in configs/data_preparation/batches/")
    print(f"To submit to SLURM:")
    print(f"  /scripts/data_preparation/submit_collection.sh {base_config} {batch_size}")