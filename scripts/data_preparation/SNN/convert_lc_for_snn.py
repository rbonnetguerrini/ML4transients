...
def convert_single(input_path, output_path):
    ...
    # For each SNID, find MJD of maximum FLUXCAL
    max_mjd = des_phot.groupby('SNID').apply(lambda df: df.loc[df['FLUXCAL'].idxmax(), 'MJD'])
    max_mjd = max_mjd.rename('MJD_max').reset_index()
    des_phot = des_phot.merge(max_mjd, on='SNID', how='left')
    # Keep only rows within [-30, +100] days of max
    des_phot['dt_max'] = des_phot['MJD'] - des_phot['MJD_max']
    before_cut = len(des_phot)
    des_phot = des_phot[(des_phot['dt_max'] >= -30) & (des_phot['dt_max'] <= 100)].reset_index(drop=True)
    print(f"  Applied time-window cut: removed {before_cut - len(des_phot)} rows outside [-30, +100] days of max brightness")
    # Drop helper columns
